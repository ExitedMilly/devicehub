...
import { Icon28BluetoothOutline } from '@vkontakte/icons'
...
/**
 * "Bluetooth" operblock block — on/off toggle plus the fake BLE beacons sub-feature
 * (list / add / remove) served from netsim. Uses deviceBluetoothStore + deviceBleBeaconStore.
 */
export const BluetoothSection = observer(() => {
  ...
  // Sync the current Bluetooth on/off state from the device (bluetooth button did this on open).
  useEffect(() => { void bluetoothStore.fetchStatus() }, [bluetoothStore])

  // Load the current fake BLE beacons from netsim (bluetooth button did this on open).
  useEffect(() => { void bleBeaconStore.fetch() }, [bleBeaconStore])

  return (
    <PanelSection
      active={bluetoothStore.enabled || bleBeaconStore.hasBeacons}
      icon={<Icon28BluetoothOutline />}
      title='Bluetooth'
    >
