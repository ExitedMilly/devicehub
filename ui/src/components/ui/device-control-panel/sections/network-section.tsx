...
import { SegmentedControl, Switch } from '@vkontakte/vkui'
...
/**
 * "Network" operblock block — cellular signal/speed/registration, Wi-Fi/airplane, and
 * the fake Wi-Fi scan results sub-feature. Uses the per-device deviceNetworkStore.
 */
export const NetworkSection = observer(() => {
  const networkStore = useInjection(CONTAINER_IDS.deviceNetworkStore)

  // Sync fake-scan on/off state (network button did this on open).
  useEffect(() => { void networkStore.fetchFakeScanState() }, [networkStore])

  return (
    <PanelSection
      active={networkStore.fakingActive === true}
    ...
    >
      ...
      {/* ============== Fake Wi-Fi networks (sub-feature) ============== */}
