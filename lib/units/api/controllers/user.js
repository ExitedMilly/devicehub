            ...
            if (lockingSuccessed) {
                ...
                req.options.push.send([
                    ...
                    wireutil.envelope(new wire.GroupMessage(
                        ...
                        usage,
                        // adbKeys is optional: a user document only grows the field once a key
                        // is added, so it is absent for every user who never added one. Reading
                        // it unguarded threw a TypeError here and the catch below reported it as
                        // "Failed to take control", which meant no device could ever be booked.
                        // Guarded in code rather than backfilled in the database, because the
                        // next user created would arrive without the field again.
                        (req.user.adbKeys || [])
