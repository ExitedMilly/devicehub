...
function createUser(req, res) {
    ...
    // Password comes ONLY from the body (never query/path) so it is not exposed
    // in URLs or access logs. It is required for the admin-panel local-auth flow.
    const password = apiutil.getBodyParameter(req.body, 'password')
    if (!password) {
        apiutil.respond(res, 400, 'Bad request (password is required)')
        return
    }
    // Pass privilege as undefined so the model's existing default applies
    // (first-ever user becomes ADMIN, everyone else USER).
    UserModel.createUser(email, name, req.user.ip, undefined, password).then(function(/** @type {any} */ stats) {
