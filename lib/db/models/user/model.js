...
export const createUser = async(email, name, ip, privilege, password) => {
    const rootGroup = await GroupModel.getRootGroup()
    const adminUser = await loadUser(rootGroup?.owner.email)
    // Hash the password only when one is supplied (non-empty string). Users
    // created via other auth providers (e.g. mock) pass no password and must
    // not get a password field, so they keep working unchanged.
...
// dbapi.verifyPassword = function(email, plainPassword)
export const verifyPassword = async(email, plainPassword) => {
    if (!plainPassword) {
        return false
    }
    const user = await loadUser(email)
    if (!user || !user.password) {
        // No such user, or the user has no password (created via another auth
        // provider). Return false rather than throwing so the caller can issue a
        // uniform 401 without leaking which case failed.
        return false
    }
